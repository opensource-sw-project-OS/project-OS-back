...
이 템플릿은 클라이언트와 서버 간의 안전하고 구조적인 통신을 위한 **REST API 구조 예시**입니다. 프론트엔드와 백엔드가 함께 작동하며, JWT 인증, 감정 분석 기반 소비 기록 저장, 데이터 시각화 등을 포함합니다.
...
| 구분 | 파일명 | 설명 |
|------|--------|------|
| 🔧 서버 | `server_RestAPI_templete.js` | Node.js 기반 Express 서버, MySQL 연동 및 API 처리 |
| 🌐 클라이언트 | `client_RestAPI_templete.html` | HTML 내 JS로 작성된 REST API 호출 템플릿 (Fetch API 사용) |

---

## 🌐 클라이언트 구조 (client_RestAPI_templete.html)

### 🔒 사용자 인증 관련 변수

```js
let user_token = null;  // 로그인 시 발급받은 토큰 저장 (전역 변수)
```
...
| 경로 | 메서드 | 설명 |
|------|--------|------|
| `/api/signup` | POST | 회원가입, `Userid`, `Userpassword` 필요 |
| `/api/login` | POST | 로그인, 토큰 발급 |
| `/api/data` | POST | OCR/감정 데이터 저장 |
| `/api/graph/emotion` | GET | 감정별 총 지출 조회 |
| `/api/graph/category` | GET | 카테고리별 총 지출 조회 |
| `/api/graph/daily` | GET | 일별 감정 및 지출 조회 |
